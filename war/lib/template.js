 ...
 *  ex) "		#{for:subList}",
 		...
 		"		#{/for:subList}"
 *  ex) '#{if:image!=""}<img src="#{image}" data="#{data}"/>#{/if:image}',
...
(function(){
	...
	function evaluate(data, tpl) {
		...
		if (tpl.indexOf("\{loop:") > -1) {
			tpl = tpl.replace(/#\{loop:([\d]+)-([\d]+)\}([\s\S]*?)#\{\/loop\}/gm, function(full, start, end, looptpl){
				start = parseInt(start);
				end = parseInt(end);
				var _listTpl = [];
				for(var i=start;i<=end;i++) {
					_listTpl.push(evaluate(data, looptpl));
				}
				return _listTpl.join("");
			});
		}
		if (tpl.indexOf("\{if:") > -1) {
