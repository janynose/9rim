...
daum.extend(daum,{
	...
	Array: {
		...
		compact: function(a){
			if(!a)return [];
			for(var i=0, ret=[], l = a.length; i<l; i+=1){
		...
		indexOf: function(/*a, _find*/){
			if([].indexOf){
				...
				return function(a, _find){
					for(var i=0,l=a.length;i<l;i+=1){if(a[i] === _find){return i;}}
					return -1;
				};
			}
		}(),
	...
	Element: {
		...
		hasClassName: function(el, cname){
			var cnames = daum.String.trim(daum.$(el).className).replace(/\s+/g,' ').split(' '),
				needles = daum.String.trim(cname).replace(/\s+/g,' ').split(' '), ret = true;
			daum.Array.each(needles, function(item){
	...
	Event: {
		 ...
		 * <pre name="code" class="js:nogutter:nocontrols">
		 * var eid = daum.Event.addEvent(document, "keydown", handleKeyDown);
		 ...
		 * <pre name="code" class="js:nogutter:nocontrols">
		 * var eid = daum.Event.removeEvent(document, "keydown", function(){….});
		 * daum.Event.stopObserving(eid);
		 * </pre>
		 * @function
	...
	$A: function(unarray){
	    ...
	    var u = (typeof unarray=='string' && (daum.ie ||daum.op)) ? unarray.split('') : unarray, result;
	    try{
	    	result = Array.prototype.slice.call(u);
	    }catch(e){
    	    for(var i=0, result=[], l=unarray.length; i<l;i+=1){
	...
	nativeExtend: function(){
		var natives = [[daum.Object, Object],
		               ...
		               [daum.Function, Function.prototype]], i, nl = natives.length;
	 ...
	 * @param Object _options 옵션값(JSON)
...
daum.extend(daum, {
	...
	createElement: function(cond, attr){
		var tag, opts = '', stack = daum.HTMLStack, ret, tagName, tagSupp, temp, i;
		if(!attr){
	...
	urlParameter : function(){
		for(var r={}, t=[],	a=location.search.substr(1).split('&'),i=0,l=a.length;i<l;i+=1){
			t=a[i].split("=");r[t[0]] = t[1];
		}
		return r;
	}(),
	...
	xmlToObject : function(xmlDocument){
		...
		builder = function(node){
			for(var obj = {}, cNodes = daum.getChildElements(node), name, value, i=0,l=cNodes.length;i<l;i+=1){
...
daum.extend(daum.Array,{
	 ...
	 */
	copy : function(a){
		for(var i=0,clone = [],p,l=a.length; i<l; i+=1){
	...
	map : function(a, f){
		...
		for (var b = [], i = 0, l = a.length; i < l; i+=1){
			b[i] = f(a[i], i);
		}
		return b;
	},
...
//	},
	...
	uniq : function(a){
		var ret = [], i;
		for(var i=0,ret=[],l=a.length;i<l;i+=1){
			daum.Array.contains(ret, a[i]) || ret.push(a[i]);
		}
		return ret;
	},
	/**
...
daum.extend(daum.Browser,{
	...
	/**
	 * 브라우저간의 팝업 사이즈를 조절하기 위해 값을 셋팅한다.
	 * @name daum.Browser.setOffset
	 * @private
	 */
	setOffset : function(){
		var br = daum.Browser, ow = 0, oh = 0;
		if(br.ie_sv1)	{ ow = 10; oh = (br.infopath) ? 58 : 29; }
		else if(br.ie7) { ow = 10; oh = 81; }
		else if(br.etc) { ow = (br.mac) ? 0 : 6; oh = (br.mac) ? 68 : 54; }
		else if(br.ff2) { ow = (br.mac) ? 0 : 6; oh = (br.mac) ? 18 : (br.infopath) ? 54 : 49; }
		else if(br.ff3) { ow = (br.mac) ? 0 : 8; oh = (br.mac) ? 68 : (br.infopath) ? 85 : 75; }
		else if(br.sf) { ow = (br.mac) ? 0 : 4; oh = (br.mac) ? 23 : 27; }
		else if(br.ns) { ow = (br.mac) ? 0 : 6; oh = (br.mac) ? 18 : 54; }
		else if(br.op) { ow = (br.mac) ? 0 : 9; oh = (br.mac) ? 36 : 49; }
		br.offset.width = ow;
		br.offset.height = oh;
	}(),
	/**
...
daum.extend(daum.Function,{
	...
	inherit : function (f, parent, /*optional*/ members){
		...
		f.prototype.$super = function(){
			...
			pConstructor.apply(this, arguments);
			c = (c==0 || (pConstructor && pConstructor.toString().indexOf('this.$super(') == -1)) ?this.constructor.prototype.parent.length:c;
		};
		...
		if(daum.Browser.ie6){
			return function(el, src, method){
				...
				if (e.tagName.toLowerCase() == "img"){
					e.src = "http://i1.daumcdn.net/imap/defaultimg/transparent.gif";
				}
			};
		}else{
			return function(el, src, method){
